...
<div class="inline-flex gap-1 rounded-sm bg-neutral-100 p-1 dark:bg-neutral-800">
    {#each tabs as { value, Icon, label } (value)}
        <button
            onclick={() => handleAppearanceChange(value)}
            class="flex items-center rounded-sm px-3.5 py-1.5 transition-colors {appearance.value ===
            value
